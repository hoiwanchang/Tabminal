...
if (virtualKeys) {
    const handleKey = (key, btn) => {
        ...
        else if (key === 'UP') data = '\x1b[OA'; // Application Cursor Keys
        ...
        if (ctrlActive) {
            ...
            } else if (data === '[') data = '\x1b'; // Ctrl+[ = Esc
            else if (data === 'C') data = '\x03'; // Explicit Ctrl+C
            
            ctrlActive = false;
            const ctrlBtn = virtualKeys.querySelector('[data-key="CTRL"]');
        ...
        if (altActive) {
            data = '\x1b' + data;
            altActive = false;
            const altBtn = virtualKeys.querySelector('[data-key="ALT"]');
            if (altBtn) altBtn.classList.remove('active');
        }

        session.send({ type: 'input', data });
        session.mainTerm.focus(); // Keep focus on terminal
    };

    // Use click for robustness, touchstart for speed
    ...
    // Handle touchend to trigger click logic without delay if needed, 
    // but 'click' is usually fine if we preventDefault on touchstart to stop ghost clicks?
    // Actually, if we preventDefault on touchstart, 'click' won't fire.
    // So we need to call handleKey on touchstart OR touchend.
    // Let's stick to 'click' and NOT preventDefault on touchstart unless we manually handle it.
    // Better: preventDefault on 'mousedown' to stop focus loss on desktop?
    // For mobile:
    ...
    // Disable click if we handled touch
    virtualKeys.addEventListener('click', (e) => {
        // If generated by touch, it might be double? 
        // We used preventDefault on touchend, so click shouldn't fire.
    });
}
